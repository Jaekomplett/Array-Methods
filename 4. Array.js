...
Math.ceil()     // 크거나 같은 수 중 가장 작은 정수를 반환한다.
Math.floor()    // 작거나 같은 수 중 가장 큰 정수를 반환한다.

Math.max()      // 가장 큰 수를 반환한다.
Math.min()      // 가장 작은 수를 반환한다.

Math.pow(x, y)  // x의 y 제곱을 반환한다.
Math.random()   // 0과 1 사이의 난수를 반환한다.
Math.round()    // 가장 가까운 정수를 반환한다.
...
// 1. find : 인자로 받은 판별 함수를 만족하는 첫 번째 요소를 반환한다.
// 2. findIndex : 인자로 받은 판별 함수를 만족하는 첫 번째 식별자를 반환한다.
// 3. indexOf : 인자로 요소를 받아 만족하는 첫 번째 식별자를 반환한다.
